...
<<ipdw-2>>=
costras<-costrasterGen(kattegat$coords,pols,extent="points",katproj)
...
\section{Influence of Varying Coastline Complexity}

The apparent benefits of IPDW may be related to the degree of coastline complexity. In theory, IPDW should provide greater benefit the greater the coastline complexity. The following section explores this possibility by generating artifical coastlines of varying complexity using Geographic Resources Analysis Support System (GRASS).
\vspace{12pt}
\texttt{\\
R> library(spgrass6)\\
R> loc<-initGRASS("./grass64",home=getwd(),gisDbase="GRASS\textunderscore{}TEMP",override=TRUE)\\
R> matsize<-64\\
R> m<-matrix(NA,matsize,matsize)\\
R> extentraster<-raster(m,xmn=0,xmx=matsize-1,ymn=0,ymx=matsize-1)\\
R> extentraster[]<-runif(ncell(extentraster),min=1,max=matsize)\\
R> extentSGDF<-as(extentraster,"SpatialGridDataFrame")\\
R> extent.g<-writeRAST6(extentSGDF,"extent",flags=c("overwrite"))\\
R> execGRASS("g.region",rast="extent")\\
\\
fractalGen<-function(fracdim){\\
execGRASS("r.surf.fractal",output="test",dimension=fracdim,flags=c("overwrite"))\\
#execGRASS("r.info",flags="r",map="test")\\
test<-readRAST6("test",ignore.stderr=TRUE)\\
}\\
}
